...
document.addEventListener('DOMContentLoaded', () => {
  const recommendationDiv = document.getElementById('powell-recommendation');

  if (!recommendationDiv) return;

  // Wait until restaurants array is initialized by scripts.js
  const waitForRestaurants = () => {
    if (window.restaurants && window.restaurants.length > 0) {
      const randomIndex = Math.floor(Math.random() * window.restaurants.length);
      const recommendedRestaurant = window.restaurants[randomIndex];
      recommendationDiv.textContent = `Powell's Top Pick Today: ${recommendedRestaurant.title}`;
    } else {
      // Retry if restaurants not loaded yet
      setTimeout(waitForRestaurants, 100);
    }
  };
